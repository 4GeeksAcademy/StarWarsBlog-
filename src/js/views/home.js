	...
	const characters = store.characters.map(char => 
		<div key={char.uid} className="card col-3">
			...
			<div className="card-body">
				<h5 className="card-title d-flex">{char.name}</h5>

				<p className="card-text">{char.gender}</p>
				<p className="card-text">{char.hair_color}</p>

				<div className="d-flex justify-content-between">
					<Link to={`character/${char.uid}`} className="btn btn-primary">Learn More!</Link>
					<button className="btn btn-warning" onClick={() => actions.addFavorite(char.uid)}>
